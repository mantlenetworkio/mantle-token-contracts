...
import "../../contracts/Migration/MantleTokenMigratorV2.sol";
import "../../contracts/Mock/MockERC20.sol";
import "./utils/UserFactory.sol";

contract MantleTokenMigratorV2Test is Test {
    ...
    MantleTokenMigratorV2 public mantleTokenMigrator;
    ...
    function setUp() public {
        ...
        address[] memory users = userFactory.create(7);
        deployer = users[0];
        treasury = users[1];
        userOne = users[2];
        userTwo = users[3];
        ...
        tokenTwo.mint(treasury, 10000 ether);

        vm.startPrank(deployer);
        mantleTokenMigrator = new MantleTokenMigratorV2(address(tokenOne), address(tokenTwo), treasury);
        mantleTokenMigrator.unhaltContract();

        mantleTokenMigrator.giveApproverRole(approver);
        mantleTokenMigrator.giveBlacklisterRole(blacklister);
    ...
    function test_mantleTokenMigratorCannotBeInitializedWithZeroValues() public {
        ...
        vm.expectRevert(err);
        mantleTokenMigrator = new MantleTokenMigratorV2(address(0), address(tokenTwo), treasury);

        vm.expectRevert(err);
        mantleTokenMigrator = new MantleTokenMigratorV2(address(tokenOne), address(0), treasury);

        vm.expectRevert(err);
        mantleTokenMigrator = new MantleTokenMigratorV2(address(tokenOne), address(tokenTwo), address(0));
    ...
    function test_mantleTokenMigratorCorrectlyInitialized() public {
        ...
        vm.expectRevert(err);
        mantleTokenMigrator.approveMigrationRequest(userOne, 100);
        vm.stopPrank();
    }

    function test_mantleTokenMigratorRoleAssignment() public {
        // make sure role assignment is working as intended
        assertEq(mantleTokenMigrator.blacklisters(blacklister), true);

        assertEq(mantleTokenMigrator.approvers(approver), true);

        // make sure modifiers are correctly reached
        err = abi.encodeWithSignature("MantleTokenMigrator_OnlyApprover(address)", address(this));
        vm.expectRevert(err);
        mantleTokenMigrator.approveMigrationRequest(userOne, 100);

        err = abi.encodeWithSignature("MantleTokenMigrator_OnlyBlacklister(address)", address(this));
        vm.expectRevert(err);
        mantleTokenMigrator.blacklistAddress(userOne);
        vm.stopPrank();
    }

    function test_approveMigrationRequest() public {
        uint256 amountTooLargeToMigrate = 100000 ether;
        uint256 amountToMigrate = 10000 ether;
        ...
        vm.expectRevert(err);
        mantleTokenMigrator.approveMigrationRequest(userOne, amountToMigrate);
        ...
        assertEq(tokenTwo.balanceOf(address(mantleTokenMigrator)), 10000 ether);

        vm.prank(userOne);
        tokenOne.approve(address(mantleTokenMigrator), amountTooLargeToMigrate);
        ...
        err = bytes("TRANSFER_FROM_FAILED");
        vm.expectRevert(err);
        mantleTokenMigrator.approveMigrationRequest(userOne, amountTooLargeToMigrate);

        // migrate the correct amount of tokenOne
        mantleTokenMigrator.approveMigrationRequest(userOne, amountToMigrate);
        ...
        assertEq(tokenTwo.balanceOf(userOne), 10000 ether);
    ...
    function test_approveMigrationRequestBlacklist() public {
        vm.prank(blacklister);
        mantleTokenMigrator.blacklistAddress(userTwo);
        ...
        err = abi.encodeWithSignature("MantleTokenMigrator_AddressBlacklisted(address)", userTwo);
        vm.expectRevert(err);
        mantleTokenMigrator.approveMigrationRequest(userTwo, 10000 ether);
        vm.stopPrank();
    }
    ...
    function test_defundContract() public {
        ...
        vm.prank(approver);
        mantleTokenMigrator.approveMigrationRequest(userOne, 5000 ether);

        // assert balances are correct
        assertEq(tokenOne.balanceOf(address(mantleTokenMigrator)), 5000 ether);
        assertEq(tokenOne.balanceOf(userOne), 5000 ether);
        assertEq(tokenTwo.balanceOf(address(mantleTokenMigrator)), 5000 ether);
        assertEq(tokenTwo.balanceOf(userOne), 5000 ether);
        ...
        assertEq(tokenTwo.balanceOf(address(mantleTokenMigrator)), 4000 ether);
        assertEq(tokenTwo.balanceOf(treasury), 1000 ether);
    }
    ...
    function _fundContractWithTokenTwo() internal {
        ...
        tokenTwo.transfer(address(mantleTokenMigrator), 10000 ether);
        vm.stopPrank();
    }
}
